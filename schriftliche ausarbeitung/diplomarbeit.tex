...
%user generated content is important and the systems should involve it more in their pages ka 
  ...
  More and more website operators are adding methods to their sites which have the goal to increase the user engagement by allowing users to actively contribute to the content of the website.
  ...
  This process should work even if the words have spelling mistakes or are in a different form than the words of the \emph{thesaurus}.
...
In this paper \citet{Joachims:2005:AIC:1076034.1076063} analyze the behaviour of users with eye tracking technology before they click on links in a search result.
...
\section{Challenges}
  ...
  Moreover each component should be usable as a standalone system to extend existing technologies.
...
Chapter~\ref{chapter:implementation} starts with a section about the presentation of the technologies that are used for implementing the developed concept.
...
The collected data, however, showed that the number of mouse clicks on a website does not significantly differ even if the user rated it with the highest or lowest rating. %\todo{check this sentence}
...
The web browser the team of \citet{claypool2001} developed measured the time a user scrolls a website by keys and by mouse.
They compared the data of the method of the field experiment with over 80 people with the explicit user ratings.
Due to this data they found out that both the measurement of user scrolls by keys and by mouse are on their own poor indicators of interest whereas a combination of both methods is found to be a good indicator of interest.
...
This process takes time so that the user stays on the website longer and most of the time he/she has to scroll in order to see the whole content of the website.
...
For this thesis the time differences between the average times of each rating are assumed as a range for implicit ratings.
...
It would have been desirable to conduct a field experiment for the newly developed concept of the total rating but unfortunately due to the lack of time it was not possible.
...
In the context of economics the \emph{Deutsche Zentralbibliothek für Wirtschaftswissenschaften Leibniz-Informationszentrum Wirtschaft} (ZBW) provides the \emph{STW Thesaurus for Economics} (STW).
...
The \emph{levenshtein distance} calculates the minimum numbers of substitutions, insertions and deletions that are needed to change one word into another[\citet{wiki:levenshteinDistance}].
...
%definitions
The following definitions are based on the book by \citet{automata2003}.
...
Therefore after reading the letter 't' in the initial state $0^0$ the automaton can be in five different states namely $0^1$, $1^2$, $1^1$, $2^2$, $1^0$, $2^1$ and $3^2$.
...
This choice is made because a distance of more than one would yield too many false matches for such short words.
...
The overall tagging concept is to calculate for every word from the text an automata and read all words from the STW into each automata.
...
Although it needs some time to collect initial data in the beginning it generally is a good approach if the items that need to be recommended are unknown or additional information for the items would be hard to maintain.%\todo{read a second time more explanations needed?}
...
The basic idea behind knowledge-based recommendation systems is a system that has enough information about the items and the needs of the user and as such can recommend items based on matching the needs of the user and the features of the items.
...
%if no items 
%use top rated items
All of the described recommendation approaches have advantages and usually disadvantages.
Knowledge based systems require too much domain knowledge that is not accessible for user generated questions and answers and therefore knowledge based systems are not feasible for the system.
...
Due to the fact that the user generated content is not moderated and therefore the system has no information about the quality of the content the implemented system uses a collaborative filtering technique with pre filtered content based on the tags for each item.
...
\begin{definition}[Prediction]
  A prediction is a rating $r_{i,u}$ for item $i \in I$ and user $u \in U$ with $r_{i,v}$ being an empty entry in $R$.
\end{definition}
...
All of the collaborative recommendation techniques can be divided into two different stages of calculations.
...
\subsubsection{Matrix Factorization Recommendations}
The matrix factorization recommendation technique can be used to derive a set of latent factors from the user-item rating matrix.
...
If a prediction has to be calculated for an user $u \in U$ and an item $i \in I$ the probability that the group in which user u belongs is used to decide whether u likes item i or not.
...
Thus, the rating matrix that is used for calculating the matrix factorization has a lot of empty entries.
...
Therefore, the matrix factorization can be calculated with a matrix that provides more rating data.
Hence, the latent factors that are derived by the matrix factorization should be more distinctive and should result in better user similarities. 
For the part of the matrix factorization this thesis uses the \emph{singular value decomposition} which is a well known matrix factorization technique.
...
The idea behind this concept is that recommendations can be calculated based on similar items of the items that a user rated.
...
The adjusted cosine similarity is a special form of the \emph{cosine similarity} which is explained in the following definition which is based on \citet{recommender2011}.
...
Due to the fact that some users tend to rate items in general higher or lower than other users.
...
Consequently, more information about the user's interest is available before the actual user similarities are calculated with the \emph{singular value decomposition} which is explained in the following section.
...
\end{figure}
  ...
  \begin{displaymath}
    \overline{r}_{User1} = \frac{\sum\limits_{i \in \tilde{I}_{User1}}{r_{User1, i}}}{\sum\limits_{i \in \tilde{I}_{User1}} 1} = \frac{5 + 3 + 4 + 4}{4} = 4.0
  \end{displaymath}
 ...
 \end{displaymath}
  \begin{displaymath}
    {{(3-2.4)\cdot(3-2.4) + (5-3.8)\cdot(4-3.8) + (4-3.2)\cdot(3-3.2) + (1-2.8)\cdot(1-2.8)} \over {\sqrt{(3-2.4)^2 + (5-3.8)^2 + (4-3.2)^2 + (1-2.8)^2} \cdot \sqrt{(3-2.4)^2 + (4-3.8)^2 + (3-3.2)^2 + (1-2.8)^2}}} = 0.8040 
  \end{displaymath}
  \normalsize
...
\begin{definition}[Rank of a Matrix]
  ...
  Due to the fact that the rank of the linearly independent column vectors is always equal to the number of linearly independent row vectors it is also correct that the rank of matrix A is the maximum number of linearly independent row vectors of A.
\end{definition}
...
The singular value decomposition of an $m \times n$ matrix M with rank r is a factorization of the form:
\begin{displaymath}
  SVD(M) = U \Sigma V^t
\end{displaymath}
...
To sum this up, the matrix U corresponds to the columns of the matrix A and the matrix V corresponds to the rows of the matrix A[\citet{Sarwar02incrementalsingular}].
...
$\overline{r_a}$ which constitutes the average rating of user a.
...
The cosine similarity from definition~\ref{cosineSimilarity} is used to calculate the following similarities.
\begin{displaymath}
...
However, a rating matrix that is used by the singular value decomposition must not have empty entries in the matrix otherwise the singular value decomposition can not be calculated for this matrix.
...
The developed system combines five different concepts namely the question and answer system, the rating system, the tagging system and the rating system as well as the recommendation system.
...
In addition every value in Scala is an object and every function is a value.
...
The \emph{W3C Document Object Model Events} specification is a web standard and therefore implemented by almost all web browsers.
...
\subsection{RecommendationService}
  ...
  The \emph{/calculateUserPredictions} method takes a user id and an item id as parameters.
  ...
  The \emph{/generateRecommendations} method takes a user id, an amount of predictions that should be returned and a list of tags as parameters.
  This method forwards the parameters to the \emph{SVDBasedService} and returns the list of item rating pairs to the client that send the request.
...
%der item based service ist verantwortlich fuer die item based recommendation berechnungen dh er berechnet die item similarities und kann 
  ...
  The \emph{/calculateUserPrediction} method takes a user id and an item id as parameters.
...
\subsection{SVDBasedService}
  ...
  The \emph{SVDBasedService} service is responsible for computing the recommendations after the item based service has decreased the sparsity of the rating table.
  ...
  The third parameter that is passed to the service inside the request body as a post parameter is a JSON string that represents a list of tags.
  ...
  At first the tags are converted into the corresponding descriptor from the \emph{STW Standard Thesaurus Economics}(STW). %\todo{describe stw and descriptor}
...
\subsection{TaggerService}
  ...
  Otherwise a degree of one is used.%todo this has to match the concept
...
%with the tagging information it creates a new item
  ...
  The \emph{/currentItem} method is called every time a user visits an item-page by a JavaScript code that is included in the item-page.
  ...
  In addition, it sends the item text to the \emph{taggerService} to get the tags for the item if the item does not exist.
  ...
  Then the WebService forwards the data to the \emph{calculateRating} method.
...
\subsection{WebService}
  ...
  The \emph{/} and the \emph{/tagger} methods are both returning specific static HTML files from the file system.
  These methods should be deactivated for the production mode.
  The \emph{/} method returns the index.html file which is the main page for the recommendation test website.
  ...
  By sending the \emph{recommendations} type together with a user id to the \emph{/calculate} method the service sends a request to the RecommendationService for calculating all predictions for the items that the user has not yet rated.
  ...
  The \emph{/rating} method forwards all requests to the \emph{RatingService}.
...
The two user interactions that performed the best with an accuracy of approximately 70\% to the user ratings are the time a user spends on a website and the time a user scrolls.
...
Therefore, the ratings that the simulated users generate can be compared to the data from the team of \citet{claypool2001}.
  ...
  The table represents the results of the rating algorithm evaluation.
  ...
  The values of the \emph{Scroll Time} nearly match the average times that were found as rating indicators in the field experiment from \citet{claypool2001}.
  ...
  The \emph{Total Rating} which is introduced in this thesis correlates with the combined user interest of the \emph{Scroll Rating} and the \emph{Time Rating} and therefore represents the expected result.
...
Each user of the website can rate movies on a scale from 1 to 5 and is afterwards presented with movies that might be interesting for him/her.
...
Hence, an algorithm cannot be more accurate than the users rating variance for the same item.
  ...
  The recommendation comparison table represents the evaluation of the recommendation systems.
  ...
  This is a surprising outcome of the evaluation because it was expected that this technology would be more similar to the mean absolute error of the SVD based technologies. %\todo{any reasons?}
  ...
  The offline calculation time consists of calculating the average user ratings and calculating the average item rating as well as all other calculations that are needed in the SVD Three approach.
  ...
  Anyway, it does not vary widely from the MAE of the \emph{SVD Average} technique whereby the offline computation time of the \emph{SVD Concept} approach is by far the slowest.
  ...
  The evaluation of the \emph{SVD Three} technique indicates that SVD based recommendation systems generally result in good predictions.
...
%verdeutliche den teil den du selbst gemach hast und stelle die ergebnisse dar
  ...
  For this it is important to analyze the user generated content quality and to be able to identify the subject of the content automatically.
  ...
  However most of the words are in a different form or spelling mistakes are inserted.
  ...
  This concept uses the item based approach for decreasing the scarcity of the rating matrix which is used for the calculation of the singular value decomposition.
