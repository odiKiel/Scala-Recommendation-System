...
This can be explained by the fact that the user regards the overall website before he starts to evaluate the content of the website.
...
\subsection{SVDBasedService}
  ...
  The SVDBasedService service is responsible for computing the recommendations after the item based service has decreased the sparsity of the rating table.
...
%with the tagging information it creates a new item
  ...
  This JavaScript code sends the item id, the item title, the item text and the user id to the WebService which forwards these data to the rating service.
  The /currentItem method creates a new item if the current item does not exist.
  In addition, it sends the item text to the TaggingService to get the tags for the item if the item does not exist.
...
%verdeutliche den teil den du selbst gemach hast und stelle die ergebnisse dar
  ...
  In three of four tests more words were returned than the algorithm should have matched.\todo{neue info?}
  ...
  Therefore, a future optimization would be to reduce the size of the tagging base which would not just reduce the occurrence of false matches but would also decrease the time that the algorithm needs to find the matches.
