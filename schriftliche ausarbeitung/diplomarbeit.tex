...
\subsection{SVDBasedService}
  ...
  This method does not have a return value all similar user values are stored in the database for future calculations.
...
\subsection{TaggerService}
  ...
  \begin{center}
    ...
    \end{tabular} 
  \end{center}
  The \emph{tegText} method takes a text as a string as parameter. 
  ...
  For this process the service creates a finite automata for each word with degree n.
  The degree depends on the word size if the word has more than 5 letters a degree of three is used otherwise a degree of one is used.%todo this has to match the concept
  %\todo{Add Algorithm here}
  %The states are written as tupel of the form (e, i) with e denotes the current position in the word and i denotes the current levenshtein distance between the current two prefixes of v and w
  For each letter in a word four transitions are created if the levenshtein distance of the current state is smaller than n.
