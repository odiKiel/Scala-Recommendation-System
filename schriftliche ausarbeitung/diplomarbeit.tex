...
The deterministic finite automata for a word $W \in \Sigma^*$ is a \emph{levenshtein automata} with degree $n \in \mathbb{N}$ with n length of W.
\todo{optimize this, probably better if I write this as out with a face in my mind.}

The purpose of the \emph{levenshtein automata} is to decide whether $d_L(W,V)$ is smaller than a specific n with $V \in \Sigma^*$.
...
The states in Q of the \emph{levenshtein automata} denote how many letters are read i and the current \emph{levenshtein distance} between $\omega$ and W j, with $\omega$ prefix of V.
...
The function $\delta: (Q \times \Sigma \rightarrow Q)$ is mapped as follows\todo{right translation}:\\ 
\begin{math}
(i^j, \sigma) \rightarrow \begin{cases}
  (i+1)^j & \text{if correct}\\
  (i)^{(j+1)} & \text{if deletion}\\
  (i+1)^{(j+1)} & \text{if substitution or insertion}\\
  empty & \text{if j is equal to n}
\end{cases}
\end{math}

Levenshtein automata example for the word test with max \emph{levenshtein distance} 1.
\begin{tikzpicture}[->,auto,node distance=1.0cm,line width=0.2mm] 
  \node[state,initial] (q_0)   {$\begin{matrix} 0^0 & 1^1 \end{matrix}$}; 
   \node[state] (q_1) [below left=of q_0] {$\begin{matrix} 1^0 & 1^1\\ 0^1 & 2^1 \end{matrix}$}; 
   \node[state] (q_2) [below =of q_1] {$\begin{matrix} 2^0 & 2^1\\ 1^1 & 3^1 \end{matrix}$}; 
   \node[state] (q_3) [below =of q_2] {$\begin{matrix} 3^0 & 3^1\\ 2^1 & 4^1 \end{matrix}$}; 
   \node[state] (q_4) [below =of q_3] {$\begin{matrix} 4^0 & 4^1\\ 3^1 \end{matrix}$}; 
   \node[state] (q_5) [below =of q_4] {$\begin{matrix} 4^1 & \quad\\ \quad & \quad \end{matrix}$}; 
   \node[state] (q_6) [right =of q_4] {$\begin{matrix} 3^1 & 4^1\\ \quad & \quad \end{matrix}$}; 
   \node[state] (q_7) [right =of q_3] {$\begin{matrix} 2^1 & 3^1\\ 4^1 & \quad \end{matrix}$}; 
   \node[state,accepting,minimum size=4.5em](q_4) [below =of q_3] ;
   \node[state,accepting,minimum size=4.5em](q_3) [below =of q_2] ;
   \node[state,accepting,minimum size=4.5em](q_5) [below =of q_4] ;
    \path[->] 
    (q_0) edge  node {t} (q_1)
    (q_1) edge  node  {e} (q_2)
    (q_2) edge  node  {s} (q_3)
