...
These different approaches are the item-based recommendation, the singular valued decomposition (SVD) based recommendation, and the approach that is described in this thesis that uses item-based predictions to decrease the sparsity of the rating table and the SVD-based approach for recommendations.\todo{improve this}
...
They explain this hypothesis with the reason that users tend to provide inconsistent ratings when they are asked to rate the same movie at different times and therefore an algorithm cannot be more accurate than the users rating variance for the same item.
  ...
  The computer that is used for testing the different recommendation techniques is a \emph{Macbook Air} with a 1.86 GHz Intel Core 2 Duo precessor and 4 GB DDR3 Memory.
  ...
  This is a surprising\todo{can I write this?} outcome of the evaluation and cannot be explained it was expected that this technology would be more similar to the mean absolute error of the \emph{SVD Concept} technology. \todo{any reasons?}
  ...
  This should have the highest difference to the matrix that would represent the real user interest.\todo{improve this}
  ...
  The three was chosen because it represents the middle of a five point rating scale. 
  Therefore it should have the smallest mean absolute error of all ratings if the user ratings are evenly distributed.
  ...
  So, it has a very fast computation time  \dots
  ...
  It has almost the same mean absolute error as the item based approach.
  Therefore it can be concluded that the interests of the individual user from the \emph{MovieLens} database does not vary widely.
  \dots

  Conclusion
  %die evaluation sollte man wiederholen wenn mit dem system ratings gesammelt wurden die zum anwendungsgebiet passen um zu sehen ob die mean absolute error werte sich dadurch veraendern. man kann sagen, dass der svd approach mit average ratings das beste empfehlungs system ist, da es mit die besten mean absolute error werte erreicht und zudem noch performant in den computation times arbeiten kann. allerdings sollte man fuer eine webseite die keine zu genauen empfehlungen benoetigt durchaus die reine average version in erwaegung ziehen, da sie sehr gute computation times hat und einen guten mean absolute error bei der getesteten datenbank erreichen konnte.
